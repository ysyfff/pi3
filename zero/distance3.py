...
sensor_inner = DistanceSensor(26, 19, max_distance=3)
led = LED(16)
status = 'off'
led.off()
...
while True:
    ...
    if distance_outer < 45 and not trigger_outer:
        trigger_outer = True
        time_outer = nowTime()
        print time_outer, 'outer'*20
    # 触发了inner，计时并标记
    if distance_inner < 90 and not trigger_inner:
        trigger_inner = True
        time_inner = nowTime()
        print time_inner, 'inner'*20

    # 如果触发了机关，并且触发了两个机关，计算人数
    if trigger_outer and trigger_inner:
        print time_outer, '------', time_inner
